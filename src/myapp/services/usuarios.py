...
from src.myapp.schemas.UsuarioSchema import UsuarioSchemaPublic, UsuarioSchemaCadastro, UsuarioAutenticadoSchema, UsuarioAtualizacaoSchema
...
def createUsuario(cadastro: UsuarioSchemaCadastro, secao : Session):
    ...
    db_usuario = Usuario(nome=cadastro.nome, email = cadastro.email, 
                         senha=hash_senha, telefone=cadastro.telefone,
...
def autenticacao(email: str, senha: str, session: Session):
    ...
    return UsuarioAutenticadoSchema(id = user.id,
                               email= user.email,
                               nome=user.nome,
...
def atualizarUsuario(dados: UsuarioAtualizacaoSchema, secao: Session) -> Usuario:
    ...
    if dados.email is not None:
        statement = select(Usuario).where(and_(Usuario.email == dados.email, Usuario.id != dados.id))
    ...
    return UsuarioSchemaPublic(id=usuario.id, email=usuario.email, nome=usuario.nome, 
                               telefone=usuario.telefone, estado = usuario.estado)
